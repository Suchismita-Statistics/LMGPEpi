...
DRC = LMGP(data_obs = final_data, Tmax = Tfinal, N = N_obs, prob = TRUE,
                r_a = 0.002, r_b = 43,
                p_a = 795, p_b = 1478,
                iteration = iter)
...
out <- LMGP_ppc(
  ...
  data_obs = final_data,     # cols: time, 3-day MA count
  N = N_obs,
  raw_data = ebola_drc_new$Incidence,    # optional: pre-MA counts, same length/order
