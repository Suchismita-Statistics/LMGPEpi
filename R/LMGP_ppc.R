...
#' @param probs A vector of probabilities, will be used to make credible intervals in posterior predictive checks. The default is \code{0.025, 0.5, 0.975}.
#'
#' @export


LMGP_ppc <- function(fit_full, data_obs, N,
                                      raw_data = NULL,
                                      w_norm   = NULL,
  ...
  ## -- 1. Posterior draws of u (M x (Tmax+1)) and p (length M) -----------
  ...
  draws   <- rstan::extract(fit_full, pars = c("u", "p"))
  u_draws <- draws$u
  p_draws <- draws$p
  M    <- nrow(u_draws)
  Tmax <- ncol(u_draws) - 1
  ...
  ## -- 2. Map each (u, p) draw to an implied REPORTED-count trajectory ---
  ...
  wtd_quantile <- function(x, w, probs) {
    ...
    x <- x[ord]; w <- w[ord]
    cw <- cumsum(w) / sum(w)
    sapply(probs, function(pp) x[which(cw >= pp)[1]])
  }

  interval_summary <- t(apply(pred_counts, 2, wtd_quantile, w = wts, probs = probs))
  colnames(interval_summary) <- c("lo95", "median", "hi95")

  cum_pred_counts <- t(apply(pred_counts, 1, cumsum))          # M x Tmax
  cum_summary <- t(apply(cum_pred_counts, 2, wtd_quantile, w = wts, probs = probs))
  ...
  if (!is.null(raw_df)) {
    p_interval <- p_interval +
      ggplot2::geom_point(data = raw_df,
                          ggplot2::aes(x = time, y = raw, color = "Raw"),
                          shape = 17, size = 2.5, alpha = 1)
  }
  ...
  p_interval <- p_interval +
    ...
    ggplot2::labs(title = "Predicted New Infections", x = "Time", y = "New reported infections") +
  ...
  p_cum <- ggplot2::ggplot(df_cum, ggplot2::aes(x = time)) +
    ...
    ggplot2::geom_line(ggplot2::aes(y = median), color = "darkorange", linewidth = 1) +
    ggplot2::geom_point(ggplot2::aes(y = observed), color = "black", size = 2) +
    ggplot2::labs(title = "Predicted Cumulative Infections", x = "Time", y = "Cumulative reported infections") +
    ggplot2::theme_minimal(base_size = 13)

  print(p_interval)
  print(p_cum)

  list(interval = p_interval, cumulative = p_cum)
}
